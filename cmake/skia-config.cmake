...
option (SKIA_USE_SYSTEM_DEPENDENCIES "Prefer system dependencies when building Skia" OFF)

option (SKIA_USE_SYSTEM_HARFBUZZ "Link Skia against system harfbuzz library" ${SKIA_USE_SYSTEM_DEPENDENCIES})
option (SKIA_USE_SYSTEM_FREETYPE "Link Skia against system freetype2 library" ${SKIA_USE_SYSTEM_DEPENDENCIES})
option (SKIA_USE_SYSTEM_JPEG "Link Skia against system jpeg turbo library" ${SKIA_USE_SYSTEM_DEPENDENCIES})
option (SKIA_USE_SYSTEM_PNG "Link Skia against system png library" ${SKIA_USE_SYSTEM_DEPENDENCIES})
option (SKIA_USE_SYSTEM_ICU "Link Skia against system ICU library" ${SKIA_USE_SYSTEM_DEPENDENCIES})

string (MD5 options_hash "${skia_FIND_COMPONENTS} ${SKIA_USE_SYSTEM_HARFBUZZ} ${SKIA_USE_SYSTEM_FREETYPE} ${SKIA_USE_SYSTEM_JPEG} ${SKIA_USE_SYSTEM_PNG} ${SKIA_USE_SYSTEM_ICU} ${VENDOR_SIMULATOR}")
...
if (SKIA_USE_SYSTEM_FREETYPE)
	set (SKIA_SYSTEM_FREETYPE "skia_use_system_freetype2=true")
else ()
	set (SKIA_SYSTEM_FREETYPE "skia_use_system_freetype2=false")
endif ()
if (SKIA_USE_SYSTEM_JPEG)
	set (SKIA_SYSTEM_JPEG "skia_use_system_libjpeg_turbo=true")
else ()
	set (SKIA_SYSTEM_JPEG "skia_use_system_libjpeg_turbo=false")
endif ()
if (SKIA_USE_SYSTEM_PNG)
	set (SKIA_SYSTEM_PNG "skia_use_system_libpng=true")
else ()
	set (SKIA_SYSTEM_PNG "skia_use_system_libpng=false")
endif ()
if (SKIA_USE_SYSTEM_ICU)
	set (SKIA_SYSTEM_ICU "skia_use_system_icu=true")
else ()
	set (SKIA_SYSTEM_ICU "skia_use_system_icu=false")
endif ()
...
if(UNIX AND NOT APPLE)
	...
	if ("opengles2" IN_LIST skia_FIND_COMPONENTS OR "webgl" IN_LIST skia_FIND_COMPONENTS)
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_gl=true ")
	else ()
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_gl=false ")
	endif ()
	if ("opengles2" IN_LIST skia_FIND_COMPONENTS)
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_egl=true skia_gl_standard=\\\"gles\\\" ")
	else ()
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_egl=false ")
	endif ()
	if ("webgl" IN_LIST skia_FIND_COMPONENTS)
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_webgl=true skia_gl_standard=\\\"webgl\\\" ")
	else ()
		string (APPEND SKIA_GRAPHICS_IMPLEMENTATION "skia_use_webgl=false ")
	endif ()
	...
	if (VENDOR_TARGET_ARCHITECTURE STREQUAL "wasm")
		...
		foreach (flag ${emscripten_flags})
			string (APPEND SKIA_PLATFORM_FLAGS_${VENDOR_TARGET_ARCHITECTURE} ", \\\"${flag}\\\"")
		endforeach ()
		set (emscripten_flags "${VENDOR_EMSCRIPTEN_LINKER_FLAGS}")
		separate_arguments (emscripten_flags)
		foreach (flag ${emscripten_flags})
			if (NOT "${SKIA_PLATFORM_LINKER_FLAGS_${VENDOR_TARGET_ARCHITECTURE}}" STREQUAL "")
				string (APPEND SKIA_PLATFORM_LINKER_FLAGS_${VENDOR_TARGET_ARCHITECTURE} ", ")
			endif ()
			string (APPEND SKIA_PLATFORM_LINKER_FLAGS_${VENDOR_TARGET_ARCHITECTURE} "\\\"${flag}\\\"")
		endforeach ()
	endif ()

	set (SKIA_ARGS_${VENDOR_TARGET_ARCHITECTURE}
		"target_cpu=\\\"${VENDOR_TARGET_ARCHITECTURE}\\\" extra_cflags=[${SKIA_WARNING_FLAGS} ${SKIA_COMPILER_TARGET} ${SKIA_DISABLE_ARMV8_CRC32} ${SKIA_PLATFORM_FLAGS_${VENDOR_TARGET_ARCHITECTURE}}] extra_cflags_cc=[${SKIA_WARNING_FLAGS} ${SKIA_PLATFORM_FLAGS_${VENDOR_TARGET_ARCHITECTURE}}] extra_asmflags=[${SKIA_WARNING_FLAGS} ${SKIA_COMPILER_TARGET}] extra_ldflags=[${SKIA_PLATFORM_LINKER_FLAGS_${VENDOR_TARGET_ARCHITECTURE}}] ${SKIA_SYSTEM_HARFBUZZ} ${SKIA_SYSTEM_FREETYPE} ${SKIA_SYSTEM_JPEG} ${SKIA_SYSTEM_PNG} ${SKIA_SYSTEM_ICU} ${SKIA_SYSTEM_FREETYPE_INCLUDES} ${SKIA_SYSTEM_HARFBUZZ_INCLUDES} ${SKIA_GRAPHICS_IMPLEMENTATION} ${SKIA_PLATFORM_ARGS_${VENDOR_TARGET_ARCHITECTURE}}"
	)
